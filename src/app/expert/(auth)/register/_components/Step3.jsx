...
export default function Step3({ currentStep, setCurrentStep }) {
    ...
    const [selectedService, setSelectedService] = useState({ id: 1, name: 'تهران' })
    ...
    useEffect(() => {
        stepInformation(2)
            .then((res) => {
                setSelectedItems(res.data.data.workplaces)
            })
            .catch(() => {
            })
            .finally(() => {
    ...
    const addItem = () => {
        selectedService.name && selectedPart.name &&
            setSelectedItems([...selectedItems, { state: selectedService.name, city: selectedService.name, area: selectedPart.name }])
        setSelectedPart('')
    }
                ...
                <div className='flex flex-col lg:flex-row items-end w-full mb-4'>
                    <Combobox as="div" value={selectedService} onChange={setSelectedService} className='lg:w-1/2 w-full ml-[5px]' >
                        <Combobox.Label className="block sm:text-base text-sm font-medium leading-6 text-gray-900">
                            در کدام شهر تمایل به فعالیت دارید ؟
                        </Combobox.Label>
                        <div className="relative mt-2">
                            ...
                            />
                            <Combobox.Input
                                className="sm:text-sm text-xs font-medium w-full h-11 rounded-md border-0 bg-white py-1.5 pl-3 pr-10 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-secondary-500 sm:leading-6"
                    ...
                    <div className='flex items-end w-full lg:w-1/2 lg:mt-0 mt-4'>
                        <Combobox as="div" value={selectedPart} onChange={setSelectedPart} className='w-full mr-[5px]' >
                            <Combobox.Label className="block sm:text-base text-sm font-medium leading-6 text-gray-900">
                                در کدام مناطق از این شهر تمایل فعالیت دارید ؟
                            </Combobox.Label>
                            <div className="relative mt-2">
                                <Image
                                    src={Clipboard_list}
                                ...
                                <Combobox.Input
                                    ...
                                    placeholder='به طور مثال : منطقه ۱۲'
                                    autoComplete='off'
                                />
                                <Combobox.Button className="absolute inset-y-0 left-0 flex items-center rounded-r-md px-2 focus:outline-none">
                                    <ChevronUpDownIcon className="h-5 w-5 text-gray-400" aria-hidden="true" />

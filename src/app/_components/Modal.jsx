...
function Modal({ isOpen, setIsOpen, w, h, children }) {
    ...
    return (
        <div
            onClick={handleClose}
            className={`w-screen h-screen fixed top-0 right-0 bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-20 bg-black z-[1000] items-center justify-center px-5 transition-all duration-300 
        ${isOpen ? (isAnimating ? 'opacity-0 flex' : 'opacity-100 flex') : 'hidden'}`}>
            <div
                ...
                {children}
                <div
                    className="w-6 h-6 fcc rounded-full bg-red-200 text-red-500 absolute -top-1.5 -left-1.5"
                    onClick={handleClose}
                >
                    <HiX />
                </div>

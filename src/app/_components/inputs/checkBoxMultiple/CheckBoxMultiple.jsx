...
function CheckBox({ state, title, placeholder, className, id, name, required, formik, forceOpen }) {
    ...
    return (
        <div className={`${className}`}>
            <div onClick={clickHandler} className={`transition-all duration-200 min-h-[60px] relative fcc flex-col rounded-md ${getRingStyle()} ${activeInputCondition ? 'cursor-pointer' : 'cursor-not-allowed'} ${activeInputCondition ? 'bg-white' : 'bg-gray-100'}`}>
                ...
                <div className={`flex items-center justify-between absolute z-10 right-4 text-cf-300 font-medium text-base ${getIconColor()} ${(formik.values[name] !== '' || focus || forceOpen) ? 'top-2 text-sm font-normal transition-all duration-200' : ''}`}>
                    {title}
                </div>
                ...
                <ComboboxMultiple
                    ...
                    inputRef={inputRef}
                    buttonRef={buttonRef}
                    className={(formik.values[name] !== '' || focus || forceOpen) ? 'opacity-100' : 'opacity-0'}

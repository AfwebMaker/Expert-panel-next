...
function ClipModal({ isOpen, setIsOpen, w, h, children, setFiles, setUploadFiles }) {
    ...
    return (
        ...
        ${isOpen ? (isAnimating ? 'opacity-0 flex' : 'opacity-100 flex') : 'hidden'}`}>
            <div
                ...
                className={`${w} ${h} bg-gray-50 md:rounded-xl relative px-5 pb-5 pt-7 transition-all duration-300 transform ${isAnimating ? (isOpen ? 'scale-90 opacity-0' : 'scale-100 opacity-100') : 'scale-100 opacity-100'} origin-center`}>
                {children}
                <div

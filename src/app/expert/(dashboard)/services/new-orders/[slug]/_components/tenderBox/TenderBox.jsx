...
function FurtherDetailsBox({ time, minPrice, maxPrice, link }) {
    const [isOpen, setIsOpen] = useState(false);

    return (
        <div className='bg-white w-full p-5 rounded-xl flex flex-col items-start justify-center gap-y-2'>
            <div className='flex w-full items-center gap-x-2 mb-7'>
                <HiUser className='text-primary-500 text-xl' />
                <span className='text-cf-300 text-xs'>مناقصه محدود و هزینه برآورد پروژه</span>
            </div>
            <div className='w-full flex flex-col fcc mb-5'>
            ...
            <div onClick={() => setIsOpen(true)} className='bg-primary-500 hover:bg-primary-600 transition-all duration-300 w-full h-10 text-white text-sm cursor-pointer fcc gap-x-1 rounded-lg mt-5'>
